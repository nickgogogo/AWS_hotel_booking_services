...
def generate_car_price(location, days, age, car_type):
    ...
    return days * ((100 + base_location_cost) + ((car_types.index(car_type.lower()) * 20) * age_multiplier))
...
def generate_hotel_price(location, nights, room_type):
    ...
    room_types = ['standard','queen', 'king', 'deluxe']
    cost_of_living = 0
    for i in range(len(location)):
        cost_of_living += ord(location.lower()[i]) - 97

    return nights * (10 + cost_of_living + (30 + room_types.index(room_type.lower())))
...
def isvalid_city(city):
    valid_cities = [ 'new york','sheffield','los angeles', 'chicago', 'houston', 'philadelphia', 'phoenix', 'san antonio',
                    'London', 'dallas', 'san jose', 'austin', 'jacksonville', 'san francisco', 'indianapolis',
...
def isvalid_room_type(room_type):
    room_types = ['queen', 'king', 'deluxe','standard']
...
def thanks(intent_request):
    blue=try_ex(lambda: intent_request['currentIntent']['slots']['blue'])
